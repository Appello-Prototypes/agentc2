...
export function compressSnapshot(yamlText: string): string {
    ...
    for (let i = 0; i < parsed.length; i++) {
        ...
        const shouldKeep =
            isInteractive(p) || isHeading(p) || p.changed || (p.role === "img" && p.text);
        ...
        if (shouldKeep) {
            ...
            for (let j = i + 1; j < parsed.length; j++) {
                ...
                if (
                    next.isProperty &&
                    KEEP_PROPERTY_PREFIXES.some((pf) =>
                        next.raw.trim().replace("- ", "").startsWith(pf)
                    )
                ) {
                    keepIndices.add(j);
                }
    ...
    const header =
        filtered > 0
            ? `[${keptElements} interactive elements shown, ${filtered} structural elements hidden]\n`
            : "";
