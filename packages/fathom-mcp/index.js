...
server.tool(
  ...
  {
    after: z.string().optional().describe("Only return meetings after this date (YYYY-MM-DD or ISO timestamp)"),
    before: z.string().optional().describe("Only return meetings before this date (YYYY-MM-DD or ISO timestamp)"),
    limit: z.number().optional().default(50).describe("Maximum number of meetings to return (default: 50)"),
  },
  async ({ after, before, limit }) => {
    try {
      ...
      const formatted = meetings.map(m => ({
        ...
        attendees: m.calendar_invitees?.map(a => ({
          name: a.name,
          email: a.email,
