...
<Callout type="info">
    Firecrawl uses a self-hosted MCP server that communicates with Firecrawl's API. The integration
...
<Callout type="warning">
    Store `FIRECRAWL_API_KEY` securely. In production, use environment variables or encrypted
...
<Callout type="tip">
    Always respect `robots.txt` and website terms of service. Use crawl boundaries to limit scope
...
<Callout type="info">
    Firecrawl caches content to reduce API calls and improve performance. Use `cache: false` only
    when you need absolutely fresh content.
</Callout>
...
<Callout type="warning">
    Always review website terms of service and robots.txt before scraping. Use official APIs when
    available instead of scraping.
</Callout>
