...
<Callout type="info">
    MCP servers are easier to build and maintain. OAuth-native integrations provide better user
    experience and token management but require more code.
</Callout>
...
export async function GET(request: NextRequest) {
    ...
    authUrl.searchParams.set(
        "redirect_uri",
        `${process.env.NEXT_PUBLIC_APP_URL}/api/integrations/my-service/callback`
    );
...
export async function myServiceGetItem(itemId: string, connectionId: string) {
    ...
    const tokens = JSON.parse(
        decrypt(connection.credentials.encrypted, process.env.CREDENTIAL_ENCRYPTION_KEY!)
    );
