...
<Callout type="info">
    ElevenLabs integration supports both standalone TTS (for agent voice output) and conversational
    AI agents (for two-way voice conversations with MCP tool access).
</Callout>
...
<Callout type="tip">
    `ELEVENLABS_AGENT_ID` and webhook configuration are only needed for conversational AI agents
    with tool invocation. For standalone TTS, only `ELEVENLABS_API_KEY` is required.
</Callout>
...
<Callout type="tip">
    For voice interactions, keep tool calls simple and fast. Complex multi-step operations may feel
    slow in voice conversations.
</Callout>
