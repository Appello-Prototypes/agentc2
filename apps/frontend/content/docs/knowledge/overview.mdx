...
- **Table**: `rag_documents` (managed by `@mastra/pg`)
- **Indexing**: pgvector HNSW index for fast similarity search
- **Metadata**: Document ID, chunk index, source metadata
...
| `contentType` | string   | Content type (`"markdown"`, `"text"`, `"html"`) |
| `vectorIds`   | string[] | Vector IDs for chunks                           |
| `chunkCount`  | number   | Number of chunks                                |
| `embeddedAt`  | datetime | When document was last embedded                 |
...
Vector chunks are stored in `rag_documents` (managed by pgvector):
...
| `metadata`  | JSON         | Chunk metadata (documentId, chunkIndex, etc.) |
...
3. **Stores** vectors in `rag_documents`
4. **Updates** document `vectorIds` and `chunkCount`
