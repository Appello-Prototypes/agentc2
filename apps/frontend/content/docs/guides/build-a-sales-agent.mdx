...
relatedSlugs: ["core-concepts/agents", "core-concepts/integrations", "guides/connect-your-crm"]
pageType: "tutorial"
...
<Callout type="warning">
    Sales agents interact with real customers and prospects. Always use approval gates for outbound
    communications to prevent unauthorized outreach and maintain brand reputation.
</Callout>
...
<Callout type="info">
    HubSpot integration uses MCP tools, so the agent can access CRM data through standardized tool
    calls. No custom code needed.
</Callout>
...
<Callout type="warning">
    The `gmail-send-message` tool should only be used after human approval. We'll set up approval
    gates in the next step.
</Callout>
...
<Callout type="info">
    The human approval step suspends the workflow and sends a notification. A human reviewer can
    approve, reject, or modify the email draft before the workflow continues.
</Callout>
...
<Callout type="warning">
    The `requireApprovalForEmail` guardrail blocks email sending unless explicitly approved. This
    ensures no emails are sent without human review.
</Callout>
