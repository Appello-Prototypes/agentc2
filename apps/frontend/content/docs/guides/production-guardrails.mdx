...
relatedSlugs: ["agents/guardrails", "platform/security", "agents/evaluations"]
pageType: "tutorial"
...
<Callout type="warning">
    Guardrails are critical for production deployments. This guide covers defense-in-depth with multiple policy layers.
</Callout>
...
<Callout type="info">
    Org-level policies are applied first, then agent-level policies. Agent policies can only make policies stricter, not more permissive.
</Callout>
...
<Callout type="warning">
    Prompt injection detection is not perfect. Combine with output validation and monitoring for defense-in-depth.
</Callout>
...
<Callout type="tip">
    Agent-level policies are merged with org policies. Use agent policies for domain-specific requirements.
</Callout>
...
- **[Security Best Practices](/docs/platform/security)** — Learn advanced security patterns
- **[Observability](/docs/platform/observability)** — Monitor guardrails in production
- **[Compliance](/docs/platform/security)** — Ensure guardrails meet compliance requirements
