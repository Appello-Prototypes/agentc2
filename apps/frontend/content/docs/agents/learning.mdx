...
Each learning cycle is tracked as a **learning session** that progresses through collection, analysis, proposal generation, experimentation, and promotion. Sessions record how many runs were analyzed, which scorers were used, and the baseline agent version at the time of the session.
...
| ------------------- | -------------------------------------------- | ------------------------------------------- |
| `LOW_SCORE`         | Run scored below threshold                   | Quality score below 0.6                     |
...
## What Happens During a Learning Session

When a learning session runs, the platform performs the following steps automatically:

1. **Collect runs** — Gathers agent runs matching your criteria (date range, run count) and creates a reproducible dataset snapshot
2. **Extract signals** — Analyzes run traces, evaluation scores, and user feedback to identify patterns such as recurring failures, low scores, or tool errors
3. **Generate proposals** — Uses AI to produce specific improvement proposals with risk classifications and candidate agent versions
4. **Run experiments** — Executes parallel A/B tests comparing the baseline agent against candidates, collecting metrics and scores
5. **Request approval** — For medium and high-risk proposals, sends a notification to reviewers and waits for approval or rejection
6. **Promote** — Applies approved changes by creating a new agent version with the updated configuration

Each step transitions the session to the next status automatically. You can monitor progress at any point via the API.
