...
<Callout type="warning">
    **Generate secure secrets**: Always use cryptographically secure random values for `BETTER_AUTH_SECRET`. Never commit secrets to version control.
</Callout>
...
<Callout type="tip">
    **Local HTTPS**: Use `.localhost` domains for automatic DNS resolution to `127.0.0.1`. Caddy provides self-signed certificates for local HTTPS development.
</Callout>
