...
The workflow engine provides full observability, version control, and production-ready automation.
...
Each workflow has the following key properties that you configure via the API:

- **`slug`** — A unique, URL-safe identifier for the workflow (e.g., `support-triage`)
- **`name`** — A human-readable display name
- **`description`** — An optional summary of what the workflow does
- **`definitionJson`** — The workflow definition containing steps, edges, and control flow
- **`inputSchemaJson`** / **`outputSchemaJson`** — Optional JSON Schema definitions for type-safe input and output validation
- **`maxSteps`** — Maximum number of steps allowed per execution (default: 50)
- **`timeout`** — Maximum execution time in seconds
- **`retryConfig`** — Retry configuration with `maxRetries` and `backoff` settings
- **`isPublished`** — Whether the workflow is available for production execution
- **`isActive`** — Whether the workflow is enabled
- **`version`** — Auto-incremented version number, starting at 1
- **`type`** — Either `"USER"` (custom) or `"SYSTEM"` (platform-provided)

Workflows also track their **version history**, **execution runs**, **network associations**, and **test cases**, all managed through the API.
