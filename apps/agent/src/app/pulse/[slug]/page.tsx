...
export default function PulseDetailPage() {
    ...
    // Resolve pulse slug → id
    const fetchPulse = useCallback(async () => {
        try {
            const res = await fetch(`${getApiBase()}/api/pulse`);
            if (!res.ok) throw new Error("Failed to list pulses");
            const data = await res.json();
            const match = data.pulses?.find((p: { slug: string }) => p.slug === slug);
            if (match) {
                setPulse(match);
                setPulseId(match.id);
            }
        } catch (err) {
            console.error("Failed to resolve pulse:", err);
        } finally {
            setPulseLoading(false);
        }
    }, [slug]);

    // Fetch boards scoped to this pulse
    const fetchBoards = useCallback(async () => {
        if (!pulseId) return;
        try {
            const res = await fetch(`${getApiBase()}/api/community/boards?pulseId=${pulseId}`);
            const data = await res.json();
    ...
    // Fetch feed scoped to this pulse
    ...
    const handleCreate = async () => {
        if (!newBoard.name.trim() || !pulseId) return;
        setCreating(true);
        try {
            const res = await fetch(`${getApiBase()}/api/pulse/${pulseId}/boards`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
        ...
        return (
            <div className="mx-auto max-w-7xl space-y-6 p-6">
                ...
                <div className="flex gap-6">
                    <Skeleton className="hidden h-64 w-56 rounded-xl lg:block" />
    ...
    if (!pulse) {
        return (
            <div className="mx-auto max-w-7xl p-6">
                ...
                <Button variant="outline" className="mt-4" onClick={() => router.push("/pulse")}>
                    Back to Pulses
                </Button>
            </div>
        );
    ...
    return (
        <div className="min-h-screen">
            ...
            <div className="relative overflow-hidden border-b border-zinc-800/60">
                ...
                />
                <div className="relative mx-auto max-w-7xl px-6 pt-8 pb-6">
                    <div className="flex items-start justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge
                                            variant="outline"
                                            className={statusStyles[pulse.status] ?? ""}
                                        >
                                            {pulse.status}
                                        </Badge>
                                    </div>
                        ...
                        <div className="flex items-center gap-2">
                            <Button
                                variant="outline"
                                size="sm"
                                className="gap-1.5 border-zinc-700 text-zinc-400 hover:text-zinc-200"
                                onClick={() => router.push(`/pulse/${slug}/manage`)}
                            >
                                        ...
                                        <DialogContent>
                                            ...
                                            <div className="space-y-4 py-2">
                                                <div className="space-y-2">
                                                    <Label>Name</Label>
                                                    <Input
                                                        placeholder="e.g. Research Findings"
                                                        value={newBoard.name}
                                                        onChange={(e) =>
                                                            setNewBoard((p) => ({
                                                                ...p,
                                            ...
                                            {boards.map((board) => (
                                                ...
                                                >
                                                    <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-violet-500/15">
                                                        <HashIcon className="h-3 w-3 text-violet-400" />
                                                    </div>
                        ...
                        {feedLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-900/30 py-24">
                                ...
                                <p className="mb-5 max-w-sm text-center text-sm text-zinc-500">
                                    Head into a board and be the first to share something with this
                                    pulse.
                                </p>
                                {boards.length > 0 && (
                                            ...
                                            .map((board) => (
                                                ...
                                                >
                                                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-violet-500/15">
                                                        <HashIcon className="h-3.5 w-3.5 text-violet-400" />
                                                    </div>
                                                    <div className="min-w-0 flex-1">
            ...
            <div className="flex gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <div className="mb-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs">
                        ...
                        >
                            <HashIcon className="h-3 w-3 text-violet-400/70" />
                            {post.board.name}
                        </Link>
