...
import { Badge, Button, Skeleton, Stepper, StepItem, type StepStatus } from "@repo/ui";
...
function extractLinks(output: unknown, handledKeys: Set<string>): LinkItem[] {
...
function extractMetadata(output: unknown, handledKeys: Set<string>): MetadataItem[] {
    if (!output || typeof output !== "object") return [];
    const obj = output as Record<string, unknown>;
    ...
    for (const [key, val] of Object.entries(obj)) {
        ...
        if (key.endsWith("DurationMs") && typeof val === "number") {
            items.push({ label: humanizeKey(key.replace(/Ms$/, "")), value: formatDuration(val) });
            handledKeys.add(key);
        }
    }
...
function extractDecision(output: unknown, handledKeys: Set<string>): DecisionInfo | null {
...
function extractSummaries(output: unknown, handledKeys: Set<string>): SummaryItem[] {
    if (!output || typeof output !== "object") return [];
    const obj = output as Record<string, unknown>;
...
function getRemainingFields(output: unknown, handledKeys: Set<string>): Record<string, unknown> {
    if (!output || typeof output !== "object") return {};
    const obj = output as Record<string, unknown>;
