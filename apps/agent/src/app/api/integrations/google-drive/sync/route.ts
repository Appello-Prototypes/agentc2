    ...
    try {
        ...
        if (missingScopes.length > 0) {
            return NextResponse.json(
                {
                    success: false,
                    error: "Google Drive permissions not granted",
                    missingScopes
                },
        ...
        try {
            ...
            if (!result.created.includes("google-drive")) {
                return NextResponse.json(
                    {
                        success: false,
            ...
            return NextResponse.json(
                {
                    success: false,
                    error: err instanceof Error ? err.message : "Failed to sync Google Drive"
                },
                { status: 500 }
            );
        }
        ...
        return NextResponse.json(
            {
                success: false,
                error:
                    error instanceof Error ? error.message : "Failed to sync Google Drive credentials"
            },
            { status: 500 }
        );
    }
