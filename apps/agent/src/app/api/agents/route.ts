 ...
 */
async function generateUniqueAgentSlug(base: string, workspaceId: string): Promise<string> {
    let slug = base;
    let suffix = 2;
    while (
...
export async function GET(request: NextRequest) {
    try {
        ...
        const agents = await agentResolver.listForUser(userId, organizationId);
