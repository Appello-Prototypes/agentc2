...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (body.subAgents !== undefined) {
            ...
            if (JSON.stringify(existingSubAgents.sort()) !== JSON.stringify(newSubAgents.sort())) {
                changes.push(`Sub-agents: ${existingSubAgents.length} → ${newSubAgents.length}`);
            }
        }
        if (body.workflows !== undefined) {
            const existingWorkflows = existing.workflows || [];
            const newWorkflows = body.workflows || [];
            if (JSON.stringify(existingWorkflows.sort()) !== JSON.stringify(newWorkflows.sort())) {
