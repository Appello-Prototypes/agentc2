...
/**
 * GET /api/networks/[slug]/execution-triggers
 */
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
    try {
        ...
        const recentRuns = await prisma.networkRun.findMany({
            where: {
                networkId: network.id,
        ...
        for (const run of recentRuns) {
            if (!run.triggerId || lastRunMap.has(run.triggerId)) continue;
            lastRunMap.set(run.triggerId, {
                id: run.id,
                status: run.status,
        ...
        console.error("[Network Triggers] Error listing:", error);
        return NextResponse.json(
            { success: false, error: "Failed to list network triggers" },
            { status: 500 }
        );
    }
}
...
/**
 * POST /api/networks/[slug]/execution-triggers
 */
export async function POST(
    ...
    try {
        ...
        console.error("[Network Triggers] Error creating:", error);
        return NextResponse.json(
            { success: false, error: "Failed to create network trigger" },
            { status: 500 }
        );
    }
}
