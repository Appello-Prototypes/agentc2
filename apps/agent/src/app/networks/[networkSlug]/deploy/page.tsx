...
export default function NetworkDeployPage() {
    ...
    const fetchDeployments = async (entityId: string) => {
        const res = await fetch(
            `${getApiBase()}/api/deployments?entityType=network&entityId=${entityId}`
        );
    ...
    useEffect(() => {
        const fetchNetwork = async () => {
            ...
            if (data.network) {
                ...
                setVersionId(String(data.network.version));
                fetchDeployments(data.network.id);
            }
        };
        fetchNetwork();
    }, [networkSlug]);
        ...
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Promote a network version to an environment and track approvals. Deployments
                        currently record intent and traffic allocation metadata.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid gap-3 md:grid-cols-3">
                        ...
                        <Input
                            ...
                            placeholder="Traffic %"
                        />
                        <Button onClick={createDeployment} disabled={!networkId}>
                            Create deployment
                        </Button>
                    </div>
