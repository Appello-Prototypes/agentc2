...
export default function WorkflowRunsPage() {
        ...
        async (isPolling = false) => {
            try {
                ...
                const res = await fetch(
                    `${getApiBase()}/api/workflows/${workflowSlug}/runs?${params.toString()}`
                );
    ...
    const filteredRuns = useMemo(() => {
        return runs.filter((run) => {
            const normalizedStatus = run.suspendedAt ? "suspended" : run.status.toLowerCase();
            ...
            const searchTarget = `${run.id} ${JSON.stringify(run.inputJson || "")}`.toLowerCase();
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm">
                        Review workflow executions and drill into run context.
                    </p>
                </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Filter runs and select one to inspect.</CardDescription>
                    </CardHeader>
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-2">
                                {filteredRuns.map((run) => {
                                    const displayStatus = run.suspendedAt
                                        ? "suspended"
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    Inspect inputs, outputs, and step execution metadata.
                                </CardDescription>
                            </div>
                            {selectedRun && (
                                <Badge
                                    variant={statusVariant(
                                        selectedRun.suspendedAt
                                            ? "suspended"
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                    ...
                                    <div className="rounded-md border p-3 text-sm">
                                        ...
                                        <Button
                                            ...
                                            onClick={() =>
                                                router.push(
                                                    `/workflows/${workflowSlug}/traces?runId=${selectedRun.id}`
                                                )
                                            }
                                        >
                                            View trace
                                ...
                                <div className="grid gap-4 lg:grid-cols-2">
                                    <div className="rounded-md border p-3">
                                        ...
                                        <pre className="mt-2 max-h-60 overflow-auto text-xs whitespace-pre-wrap">
                                            {JSON.stringify(selectedRun.inputJson, null, 2)}
                                        </pre>
                                    </div>
                                    <div className="rounded-md border p-3">
