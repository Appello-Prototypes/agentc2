...
The Observe tab is agent-only. It queries `AgentRun` exclusively. `WorkflowRun` and `NetworkRun` are invisible -- they only exist on their siloed pages (`/workflows/[slug]/runs`, `/networks/[slug]/runs`). When an SDLC workflow dispatches and runs 5 agents, nothing appears in Observe. Additionally, agent calls *within* workflows and networks don't create `AgentRun` records at all -- they're completely invisible.
