...
const classifyStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    if (request.includes("?") || request.startsWith("what") || 
        request.startsWith("how") || request.startsWith("why")) {
      type = "question";
    } else if (request.startsWith("do") || request.startsWith("please") ||
               request.includes("need to") || request.includes("want to")) {
      type = "command";
    }
...
const finalizeStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = inputData["handle-question"] || 
                   inputData["handle-command"] || 
                   inputData["handle-statement"];
    
    const branch = inputData["handle-question"] ? "question" :
                   inputData["handle-command"] ? "command" : "statement";
...
const aggregateStep = createStep({
  ...
  execute: async ({ inputData }) => ({
    ...
    avgLength: inputData.length > 0 
      ? Math.round(inputData.reduce((sum, item) => sum + item.length, 0) / inputData.length)
      : 0,
  }),
});
...
const prepareStep = createStep({
  ...
  execute: async ({ inputData }) => 
    inputData.items.map((value, index) => ({ value, index })),
});
...
export const doWhileWorkflow = createWorkflow({
  ...
  .dowhile(
    incrementStep,
    async ({ inputData }) => inputData.count < inputData.target
  )
  .commit();
```
