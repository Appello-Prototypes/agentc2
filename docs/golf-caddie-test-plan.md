...
| K-12 | TeeOn login flaky via Playwright             | HIGH     | Open      | Cycle 3     |             | Persistent: LoginType=5 + click/type works intermittently. Playwright ref=e1 sometimes maps to body.click() instead of textbox. Connection errors (ERR_ABORTED) occur. Not agent logic.                                                                                        |
| K-13 | Agent echoes credentials in NL responses     | MEDIUM   | Open      | Cycle 4     |             | Claude Haiku echoes username/password in confirmation and error messages despite strong instructions. Guardrail catches as safety net. Model instruction-following limitation.                                                                                                 |
