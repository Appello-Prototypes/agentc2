...
| K-15 | TeeOn search form results don't render       | MEDIUM   | Open      | Cycle 6     |             | After successful login and cookie injection, TeeOn's BookATeeTimeGolferSection search form does not display results after clicking Search. Agent tried multiple approaches (direct course page, main booking page, different time selections). Works for Bay of Quinte via ViewTeeSheet path but not via search form. May require JavaScript-based form submission or AJAX call that Playwright snapshots don't capture. |
