...
| ---- | ---------------------------------------------------- | -------- | ----------- | -------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| K-01 | SDLC Bugfix workflow 3% success rate                 | CRITICAL | Root-caused | Baseline |       | **Root cause: empty repository name in webhook trigger payload.** Workflow requires `repository` in `owner/repo` format but trigger sends empty string. All 45 failed runs show: `Cursor API error 400: Failed to verify existence of branch 'main' in repository .` |
...
| K-09 | DB unique constraint on concurrent agent calls       | HIGH     | Open        | Cycle 1  |       | `Prisma agentRunTurn.create() Unique constraint failed on (runId, turnIndex)` when calling agents rapidly. Filed as ticket #12. Blocks parallel testing.                                                                                                             |
| K-10 | Classifier uses memory-recall instead of classifying | MEDIUM   | Open        | Cycle 1  |       | Test A-03: user question "How do I reset my password?" triggered memory-recall which returned cached TeeOn content instead of producing a classification JSON.                                                                                                       |
| K-11 | BigJim2 missing workflow/Fathom tools                | MEDIUM   | Open        | Cycle 1  |       | K-02/K-03: BigJim2 cannot check workflow health or query Fathom meetings. Workflow and Fathom tools not attached to agent.                                                                                                                                           |
| K-12 | Reviewer trust score tool requires pipeline runId    | LOW      | Open        | Cycle 1  |       | `calculate-trust-score` tool requires a linked pipeline run ID. Standalone reviews without a pipeline run produce incomplete output.                                                                                                                                 |
| K-13 | `workflow_sdlc_*` direct MCP tools return HTML       | MEDIUM   | Open        | Cycle 1  |       | The direct shortcut tools (`workflow_sdlc_triage_agentc2`, `workflow_sdlc_bugfix_agentc2`) return HTML instead of JSON. Workaround: use `workflow_execute` with slug.                                                                                                |
