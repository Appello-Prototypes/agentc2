...
| 4     | 2026-03-08 | 10 (8 active)   | 5/8 (63%)              | N/A                  | **4/4**          | **4/4**       | **4/4**       | **4/4**        | **4/4**          | **2/4**          | SKIPPED           | N/A          | N/A       | N/A           | **3/4**       | ~$1.00       | ~30s            | K-16 FIXED (auto-async 55s timeout), K-18 FIXED (Planner instruction). Planner 4/4, Triage 4/4. All core SDLC agents at target. Bugfix E2E still limited by Cursor API branchName (K-17). |
| 5     | 2026-03-09 | 10 (8 active)   | 6/8 (75%)              | N/A                  | **4/4**          | **4/4**       | **4/4**       | **4/4**        | **4/4**          | **3/4**          | SKIPPED           | N/A          | N/A       | N/A           | **3/4**       | ~$1.50       | ~280s           | **K-17 FIXED** (workflow def), **K-19 FIXED** (suspension). Bugfix E2E reaches human review (up from 2/4). Cursor Cloud can't `gh pr create` (read-only). Fathom MCP server unavailable for BigJim2 K-03. |
...
| K-16 | `workflow_execute` MCP tool returns HTML for some workflows | MEDIUM   | **FIXED**   | Cycle 3  | C4    | Root cause: long-running workflows (bug/feature routes) exceeded HTTP timeout, proxy returned HTML 504. Fix: added 55s auto-async timeout — tool returns `{runId, status: "running"}` and continues in background. User-error route still completes synchronously.    |
| K-17 | Bugfix E2E fails at PR creation (GitHub branch validation) | HIGH     | **FIXED**   | Cycle 3  | C5    | Root cause: `implement-wait` didn't pass `repository`, `create-pr` step depended on unreliable `branchName`. Fix: Cursor prompt now requests PR creation, `repository` passed to `implement-wait` for `detectPrFromBranch`, `create-pr` step removed. Workflow now reaches human review (up from crash). Remaining gap: Cursor Cloud `gh` CLI has read-only access, so PR must be created manually or via GitHub API. |
...
| K-19 | Background workflow execution marks suspended runs as COMPLETED | HIGH     | **FIXED**   | Cycle 5  | C5    | Root cause: `executeWorkflowInBackground()` and auto-async timeout `.then()` handler mapped all non-failed results to COMPLETED, including suspended workflows. Fix: check `result.status === "suspended"` and set `suspendedAt`/`suspendedStep` with status RUNNING. |
| K-20 | Cursor Cloud Agent cannot create PRs (read-only GitHub CLI) | MEDIUM   | Open        | Cycle 5  |       | Cursor Cloud Agent's `gh` CLI has read-only access. PR creation via `gh pr create` fails silently. Workaround: use GitHub API `create-pull-request` tool as a post-implementation step, or accept manual PR creation at `merge-review`. |
...
|| Cycle 4  | 4     | **K-16 FIXED** (auto-async 55s timeout), **K-18 FIXED** (Planner "NEVER use tools" instruction). Planner 4/4 (up from 2/4). Triage bug/feature routes now return clean JSON `{runId, status: "running"}` instead of HTML 504. Feature triage reached human review (12 steps). User-error triage still completes synchronously. | +$1.00 test cost | **Classifier 4/4, Planner 4/4, Auditor 4/4, Reviewer 4/4**, Triage 4/4, Bugfix 2/4, BigJim2 3/4. All four core SDLC agents + triage at target. |
|| Cycle 5  | 5     | **K-17 FIXED** (workflow def: Cursor creates PR, `repository` passed to `implement-wait`, `create-pr` step removed). **K-19 FIXED** (background execution properly handles suspension for human steps). `prUrl` added to `cursor-poll-until-done` output. Fathom skill added to BigJim2 `preActivatedSkills`. Bugfix E2E now reaches human review (up from crash at `create-pr`). Cursor Cloud `gh` read-only access discovered (K-20). | +$1.50 test cost | Bugfix 3/4 (up from 2/4), BigJim2 3/4. All core SDLC agents remain at 4/4. Suspension fix enables proper workflow resume. |
