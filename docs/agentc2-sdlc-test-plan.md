...
| K-18 | Planner truncated output at maxSteps=3                      | MEDIUM   | Open        | Cycle 3  |       | Planner uses 2-3 of its 3 allowed steps for tool calls (web-fetch, memory-recall), leaving no steps for full plan output on complex tasks. B-01 passes (simple bug), B-02/03/04 truncated (broader scope).                                           |     |
