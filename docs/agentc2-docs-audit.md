...
| 5   | `/docs/getting-started/architecture` | Architecture           | **MIXED**      | System architecture: monorepo structure, Caddy proxy, three Next.js apps, shared packages. Includes internal file paths (`packages/mastra/src/agents/resolver.ts`), request flow, and environment variables. Architecture is public but references internal code structure. |
